...
- SSE: `ai_message_token` · `diagnosis_drafting` / `competitor_lens_drafting` / `strategy_drafting` / `creative_drafting` · `done {diagnosis?, competitor_lens?, strategy?, creative?}` (null = unchanged). Diagnosis/strategy are `{diagnoses|strategies: [{ad_id, ad_name, *_html}]}` — `ad_id:'combined'` is the roll-up doc (present for ≥2 ads or create path; default canvas selection). Competitor lens is one HTML string. Creative is the structured tree: `new {campaign, adset, ads[]}` or `tune [{campaign, adset, ad}]`; image slots hold variants with `image_url` (existing Meta asset) or `data_uri` (generated base64).
